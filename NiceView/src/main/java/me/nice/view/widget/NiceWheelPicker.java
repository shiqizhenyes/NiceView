...
public class NiceWheelPicker<V> extends View {

    private final String TAG = NiceWheelPicker.class.getSimpleName();
    ...
    protected onSelectedListener<NiceWheelPicker, V> listener;

    private NiceWheelAdapter<V> adapter = new NiceWheelAdapter<>();
        ...
        public void run() {
            ...
            if (scroller.isFinished() && !isForceFinishScroll) {
                ...
                if (null != onNiceWheelChangeListener) {
                    onNiceWheelChangeListener.onWheelSelected(position);
                    onNiceWheelChangeListener.onNiceWheelScrollStateChanged(SCROLL_STATE_IDLE);
                }
            }
            ...
            if (scroller.computeScrollOffset()) {
                ...
                if (onItemSelectedListener != null) {
                    onItemSelectedListener.onCurrentItemOfScroll(position);
                }

                Log.d(TAG, "继续滚动 position"  + position  + " scrollOffsetY " + scrollOffsetY );
    ...
    public NiceWheelPicker(Context context) {
        this(context, null);
    }

    public NiceWheelPicker(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.NiceWheelPicker);

        mItemTextSize = a.getDimensionPixelSize(R.styleable.NiceWheelPicker_nice_wheel_item_text_size,
                getResources().getDimensionPixelSize(R.dimen.WheelItemTextSize));
        mVisibleItemCount = a.getInt(R.styleable.NiceWheelPicker_nice_wheel_visible_item_count, 7);
        selectedItemPosition = a.getInt(R.styleable.NiceWheelPicker_nice_wheel_selected_item_position, 0);
        hasSameWidth = a.getBoolean(R.styleable.NiceWheelPicker_nice_wheel_same_width, false);
        textMaxWidthPosition = a.getInt(R.styleable.NiceWheelPicker_nice_wheel_maximum_width_text_position, -1);
        maxWidthText = a.getString(R.styleable.NiceWheelPicker_nice_wheel_maximum_width_text);
        mSelectedItemTextColor = a.getColor(R.styleable.NiceWheelPicker_nice_wheel_selected_item_text_color, -1);
        mItemTextColor = a.getColor(R.styleable.NiceWheelPicker_nice_wheel_item_text_color, 0xFF888888);
        mItemSpace = a.getDimensionPixelSize(R.styleable.NiceWheelPicker_nice_wheel_item_space, getResources().getDimensionPixelSize(R.dimen.WheelItemSpace));
        isCyclic = a.getBoolean(R.styleable.NiceWheelPicker_nice_wheel_cyclic, false);
        hasIndicator = a.getBoolean(R.styleable.NiceWheelPicker_nice_wheel_indicator, false);
        mIndicatorColor = a.getColor(R.styleable.NiceWheelPicker_nice_wheel_indicator_color, 0xFFEE3333);
        mIndicatorSize = a.getDimensionPixelSize(R.styleable.NiceWheelPicker_nice_wheel_indicator_size, getResources().getDimensionPixelSize(R.dimen.WheelIndicatorSize));
        hasCurtain = a.getBoolean(R.styleable.NiceWheelPicker_nice_wheel_curtain, false);
        mCurtainColor = a.getColor(R.styleable.NiceWheelPicker_nice_wheel_curtain_color, 0x88FFFFFF);
        hasAtmospheric = a.getBoolean(R.styleable.NiceWheelPicker_nice_wheel_atmospheric, false);
        isCurved = a.getBoolean(R.styleable.NiceWheelPicker_nice_wheel_curved, false);
        mItemAlign = a.getInt(R.styleable.NiceWheelPicker_nice_wheel_item_align, ALIGN_CENTER);
        a.recycle();
...
//        setSelectedItemPosition(findIndexOfDate(date));
    ...
    protected void onDraw(Canvas canvas) {
        if (null != onNiceWheelChangeListener) onNiceWheelChangeListener.onWheelScrolled(scrollOffsetY);
        ...
        if (isEnabled()) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    Log.d(TAG, "scrollOffsetY " + scrollOffsetY + " velocity " +  velocity +  " minFlingY " + minFlingY  +
                            " maxFlingY " + maxFlingY);
                    if (Math.abs(velocity) > minimumVelocity) {
                        scroller.fling(0, scrollOffsetY,
                                0, velocity, 0, 0,
                                minFlingY, maxFlingY);
                        Log.d(TAG, "startScroll scrollOffsetY" + scrollOffsetY + " mItemHeight " + mItemHeight
                                + " Distance getFinalY"+
                                computeDistanceToEndPoint(scroller.getFinalY() % mItemHeight));
                        scroller.setFinalY(scroller.getFinalY() +
                                computeDistanceToEndPoint(scroller.getFinalY() % mItemHeight));
                        Log.d(TAG, "fling scrollOffsetY" );
                    } else {
                        Log.d(TAG, "startScroll scrollOffsetY" + scrollOffsetY + " mItemHeight " + mItemHeight + " Distance "+
                                computeDistanceToEndPoint(scrollOffsetY % mItemHeight));
                        Log.d(TAG, "startScroll 求余数： " + (scrollOffsetY % mItemHeight) );
                    ...
                    if (scroller.getFinalY() < minFlingY) {
    ...
    public void scrollTo(final int itemPosition) {
        ...
        if (itemPosition != getCurrentItemPosition()) {
            ...
            Log.d(TAG, "differencesLines " + differencesLines  + " scrollOffsetY "  + scrollOffsetY
                    + " newScrollOffsetY " +newScrollOffsetY  + " mItemHeight" + mItemHeight);
    ...
    public void setAdapter(NiceWheelAdapter adapter) {
    ...
    public void setOnNiceWheelChangeListener(OnNiceWheelChangeListener onNiceWheelChangeListener) {
        this.onNiceWheelChangeListener = onNiceWheelChangeListener;
    }
